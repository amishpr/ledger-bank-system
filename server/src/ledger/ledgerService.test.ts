...
import { createAccount, getAccount, postTransaction, reverseTransaction } from "./ledgerService.js";
...
describe("postTransaction", () => {
  it("rejects a transaction where debits and credits don't balance", async () => {
    const asset = await createAccount({ name: "Checking", type: "ASSET" });
  ...
  it("refuses to post an entry that would take an asset account negative", async () => {
    ...
    const balance = await getAccount(asset.id);
  ...
  it("replays an idempotent request instead of double-posting", async () => {
    ...
    const balance = await getAccount(asset.id);
    expect(balance.balanceMinor).toBe(1000n); // not 2000n - no double posting
  });
...
describe("reverseTransaction", () => {
  it("restores the prior balance and voids the original", async () => {
    ...
    expect((await getAccount(asset.id)).balanceMinor).toBe(3800n);

    const reversal = await reverseTransaction(transfer.transaction.id);
    expect((await getAccount(asset.id)).balanceMinor).toBe(5000n);

    const original = await prisma.transaction.findUniqueOrThrow({ where: { id: transfer.transaction.id } });
